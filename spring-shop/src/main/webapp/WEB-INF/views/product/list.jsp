...
function removeCheckedProducts() {
	/*
		...
		document.querySelectorAll("input[type=checkebox]);
			- 태그명이 input이고, type속성값이 checkbox인 모든 엘리먼트를 선택한다.
		document.querySelectorAll("input[type=checkebox][name=no]);
			- 태그명이 inputdlrh, type속성값이 checkebox이고 name속성값이 no인 모든 엘리먼트를 선택한다.
			document.querySelectorAll("input[type=checkebox][name=no]:checked);
				- 태그명이 inputdlrh, type속성값이 checkebox이고 name속성값이 no이고,
					체크상태가 checked인 모든 엘리먼트를 선택한다.
	*/
	// 체크된 체크박스를 모두 선택한다.
