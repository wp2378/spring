...
<script type="text/javascript">
	...
	function showUserInfo(id) {
		...
		xhr.onreadystatechange = function() {					// 2. xhr객체의 readyState값이 변경될 때 마다 실행되는 이벤트핸들러 함수를 등록한다.
			if(xhr.readyState == 4 && xhr.status == 200) {		// 5. readyState=4:응답이 왔다. status=200:성공 = 서버에서 응답이 성공적으로 왔다.
				let text = xhr.responseText;					// 6. xhr의 responseText 프로퍼티에서 텍스트응답데이터를 조회한다.
				console.log(text);
				let user = JSON.parse(text);					// 7. user = {no: 1005, id: 'aaaa', name: 'aaaaaaaaaaaaaaaaaaaaa', tel: '02-5555-5555', email: 'hong@gmail.com', …} 역직렬화 되어 저장되어있다.
				console.log(user);
