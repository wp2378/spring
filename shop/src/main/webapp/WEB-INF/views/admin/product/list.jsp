...
/*	function showProductInfo(no) {
	...
	async function submitForm() {
		...
		console.log(data);		// {no: '154', categoryNo: '904', name: 'asdaas', companyNo: '1003', price: '111', …}
		
		
		let jsonText = JSON.stringify(data);
		console.log(jsonText);	// {"no":"154","categoryNo":"904","name":"asdaas","companyNo":"1003","price":"111","stock":"55","description":"신규상품입니다"}
		...
		if (response.ok) {
			...
			// select.options는 셀렉터박스의 모든 옵션을 배열로 변환한다.
			// select.selectedIndex는 셀렉터박스의 옵션 중에서 현재 선택한 옵션의 index를 반환한다.
			// select.options[select.selectIndex]는 셀렉터박스의 옵션 중에서 현재 선택한 옵션 엘리먼트를 반환한다.
