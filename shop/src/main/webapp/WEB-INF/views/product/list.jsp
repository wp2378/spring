							...
							<%--
								<c:forEach var="product" items="${products }" varStatus="status">
											...
											var			- items로 조회한 콜렉션, 배열 객체에 저장된 객체들을 순서대로 하나씩 꺼냈을 때
														  그 객체가 대입되는 변수명이다. 
											items		- 반복처리 대상이 되는 콜랙션, 배열 객체를 컨트롤러에서 모델에 저장할 때 이름을 지정하면,
														  그 이름으로 저장된 콜렉션, 배열객체를 조회한다.
											varStatus	- forEach태그로 반복작업을 수행할 때 반복시마다 현재 반복작업의 상태정보가 저장되는 객체를 있는데
														  그 객체가 대입되는 변수명이다.
														 해당 작업의 주요 프러퍼티
														 		...
														 		last  : 현재 반복처리중인 작업이 마지막 작업인 경우 true를 출력한다.
								varStatus <c: />태그에서 db에 저장되 있는 번호 외에 사용자지정 번호를 정할때 씀
								${status.index } : 0,1,2,3,4 순으로 나열
								${status.count } : 1,2,3,4,5 순으로 나열
								${status.first } : 순서의 첫번째면 true를 반환 나머지는 false
								${status.last }  : 순서의 마지막이면 true를 반환 나머지는 false
