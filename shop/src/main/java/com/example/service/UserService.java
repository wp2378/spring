...
import com.example.exception.AlreadyUserEmailExcetion;
import com.example.exception.AlreadyUserIdException;
 ...
 * 			* final 키워드가 지정되어 잇는 멤버변수는 반드시 초기화가 필요하다.
 * 			* final 키워드가 지정되어 있는 멤버변수를 초기화라는 방법은 2가지가 있다.
 * 				- 멤버변수에 직접 값을 대입하기
...
public class UserService {
	 ...
	 * 			2. 이메일로 회원정볼르 조회한다. 회원정보가 존재하면 예외를 발생시킨다.
	...
	public void registerUser(UserRegisterForm form) {
		...
		if(foundUser != null) {
			throw new AlreadyUserIdException("["+form.getId()+"]는 이미 사용중인 아이디입니다.");
		}
		
		foundUser = userMapper.getUserByEmail(form.getEmail());
		if(foundUser != null) {
			throw new AlreadyUserEmailExcetion("["+form.getEmail()+"]는 이미 사용중인 이메일입니다.");
		}
		
		User user = form.toUser();
		// 비밀번호 암호화
		String secratPassword = passwordEncoder.encode(user.getPassword());
		user.setPassword(secratPassword);
